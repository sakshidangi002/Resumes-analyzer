...
# Requirements: ONE file, copied verbatim.
#
# This used to stitch requirements-all.txt together from two source files,
# stripping an "Attendance Management" marker out of one and shared-auth lines
# out of the other. That merge existed because the dependencies were split
# across files while the server has only ever had ONE virtualenv - and the
# split was the bug, not the thing to work around. There is now a single
# requirements.txt, so the server gets exactly what a developer gets.
#
# requirements-all.txt is still written, under that name, so existing
# setup-server.bat and any documented instructions keep working.
$reqSource = Join-Path $Root "requirements.txt"
$header = @(
    "# AUTO-GENERATED by scripts\package-for-server.ps1 - a copy of the repo's",
    "# requirements.txt. Edit that file, not this one.",
    "#",
...
$combined = ($header + (Get-Content $reqSource -Encoding UTF8)) -join "`r`n"
