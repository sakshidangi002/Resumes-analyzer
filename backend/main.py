...
_EDUCATION_DEGREE_WORDS = {
    ...
    "diploma", "phd", "mba", "btech", "mtech", "bsc", "msc", "bba", "business administration", "computer application", "arts", "science",
...
def _passes_technical_skill_output(raw: str, vocab: set[str]) -> bool:
    ...
    if len(norm_low.split()) == 1:
        blocked_single_words = {
            "business", "executive", "lead", "leadership", "thinking", "handling", "social", "media", "design", "designing", "solution", "solutions",
                            ...
                            if not is_template:
                                ...
                                if llm_result.get("experience_years") is not None:
                                    try:
                                        ...
                                        logger.debug("experience_years is not numeric", exc_info=True)
...
def estimate_experience_years_from_text(resume_text: str) -> float:
    ...
    if m:
        try:
            ...
            logger.debug("experience value is not numeric", exc_info=True)
    ...
    if m_mos:
        try:
            ...
            logger.warning("round failed", exc_info=True)
